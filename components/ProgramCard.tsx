  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0 pr-4">
        <h3 className={`text-base font-bold leading-tight mb-1 truncate ${isActive ? 'text-[#064e3b]' : 'text-slate-800'}`}>
          {program.name}
        </h3>
        <div className="flex items-center text-slate-500 text-xs">
          <span className="truncate tracking-wide">{program.venue}</span>
        </div>
      </div>

      <div className="flex-shrink-0 flex items-center">
        {program.distance !== undefined && (
          <span className={`text-[10px] font-black px-2.5 py-1.5 rounded-xl whitespace-nowrap transition-colors uppercase tracking-wider ${isActive ? 'bg-[#064e3b] text-white shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200'}`}>
