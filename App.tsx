...
const App: React.FC = () => {
  ...
  const LOGO_URL =
    "https://crm.pcirealestate.site/wp-content/uploads/2026/01/Logo-DTQ-app.png";
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.warn(
            "Initial location fetch failed, using fallback/waiting for watch.",
            error,
          );
        },
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      ...
      const effectiveLocation = searchCenter ||
  ...
  const handleMarkerClick = useCallback(
    (program: Program) => setSelectedProgram(program),
    [],
  );
  ...
  const effectiveLocationForPrayers = searchCenter ||
    userLocation || { lat: 33.6844, lng: 73.0479, name: "Islamabad" };
    ...
    <APIProvider apiKey={API_KEY}>
      <div className="relative h-screen w-full bg-[#fdfcf6] flex flex-col md:flex-row overflow-hidden pb-20 md:pb-0">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 h-[72px] bg-white border-t border-slate-100 flex items-center justify-between px-4 z-[3000] pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
          ...
          >
            <div
              className={`w-12 h-10 flex items-center justify-center rounded-full transition-all duration-300 ${activeTab === "events" ? "bg-[#064e3b]/10 scale-105" : "hover:bg-slate-50"}`}
            >
              <Icons.MapPin
                className={
                  activeTab === "events" ? "w-6 h-6" : "w-5 h-5 opacity-70"
                }
              />
            </div>
            <span
              className={`text-[10px] ${activeTab === "events" ? "font-black" : "font-bold"} uppercase tracking-[0.15em]`}
            >
              Map
            </span>
          </button>
          ...
          >
            <div
              className={`w-12 h-10 flex items-center justify-center rounded-xl transition-all duration-300 ${activeTab === "prayers" ? "bg-[#064e3b]/10 scale-105" : "hover:bg-slate-50"}`}
            >
              <Icons.Clock
                className={
                  activeTab === "prayers" ? "w-6 h-6" : "w-5 h-5 opacity-70"
                }
              />
            </div>
            <span
              className={`text-[10px] ${activeTab === "prayers" ? "font-black" : "font-bold"} uppercase tracking-[0.15em]`}
            >
              Prayers
            </span>
          </button>
          ...
          >
            <div
              className={`w-12 h-10 flex items-center justify-center rounded-xl transition-all duration-300 ${activeTab === "quran" ? "bg-[#064e3b]/10 scale-105" : "hover:bg-slate-50"}`}
            >
              <Icons.BookOpen
                className={
                  activeTab === "quran" ? "w-6 h-6" : "w-5 h-5 opacity-70"
                }
              />
            </div>
            <span
              className={`text-[10px] ${activeTab === "quran" ? "font-black" : "font-bold"} uppercase tracking-[0.15em]`}
            >
              Quran
            </span>
          </button>
          ...
          >
            <div
              className={`w-12 h-10 flex items-center justify-center rounded-xl transition-all duration-300 ${activeTab === "about" ? "bg-[#064e3b]/10 scale-105" : "hover:bg-slate-50"}`}
            >
              <Icons.Info
                className={
                  activeTab === "about" ? "w-6 h-6" : "w-5 h-5 opacity-70"
                }
              />
            </div>
            <span
              className={`text-[10px] ${activeTab === "about" ? "font-black" : "font-bold"} uppercase tracking-[0.15em]`}
            >
              About
            </span>
          </button>
            ...
            >
              <div className="hidden md:flex flex-col items-center mb-10 animate-in fade-in slide-in-from-top duration-700">
                ...
                <div className="flex gap-4 mt-6">
                  <button
                    onClick={() => setActiveTab("events")}
                    className={`px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === "events" ? "bg-[#064e3b] text-white shadow-lg" : "text-slate-400 hover:bg-slate-50"}`}
                  >
                    Events Map
                  </button>
                  <button
                    onClick={() => setActiveTab("prayers")}
                    className={`px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === "prayers" ? "bg-[#064e3b] text-white shadow-lg" : "text-slate-400 hover:bg-slate-50"}`}
                  >
                    Prayer Times
                  </button>
                  <button
                    onClick={() => setActiveTab("quran")}
                    className={`px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === "quran" ? "bg-[#064e3b] text-white shadow-lg" : "text-slate-400 hover:bg-slate-50"}`}
                  >
                    Quran
                  </button>
                  <button
                    onClick={() => setActiveTab("about")}
                    className={`px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === "about" ? "bg-[#064e3b] text-white shadow-lg" : "text-slate-400 hover:bg-slate-50"}`}
                  >
              ...
              <div
                className={`pointer-events-auto glass-midnight rounded-[1.5rem] p-4 shadow-2xl md:bg-white md:shadow-none md:p-0 md:border-none duration-500 transition-all ${selectedProgram ? "opacity-0 scale-95 pointer-events-none translate-y-[-20px]" : "opacity-100 scale-100"}`}
              >
                <div className="md:hidden flex justify-between items-center py-2 mb-4 px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-white/90 uppercase tracking-[0.2em] leading-none">
                        Nearby
                      </span>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 px-1 custom-scroll snap-x">
                  <button
                    ...
                    className={`snap-center flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold transition-all border ${
                      !searchCenter
              ...
              <div className="hidden md:flex flex-col flex-1 overflow-hidden">
                <div className="flex items-center justify-between mb-6 px-1">
                  ...
                  <div className="flex items-center gap-2">
                    <Icons.Crescent />
                    <span className="text-xs font-bold text-[#065f46] px-3 py-1 bg-[#065f46]/10 rounded-full">
                      {loading ? "..." : filteredPrograms.length} active
                    </span>
                  </div>
                </div>
                  ...
                  {loading ? (
                    <div className="space-y-4 animate-pulse">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="bg-white p-4 rounded-xl border border-slate-100 flex gap-4"
                        >
                ...
                >
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-xl border border-slate-100">
                      ...
                      <p className="text-[#0f172a] font-black text-xl mb-2">
                        Finding light...
                      </p>
                      <p className="text-slate-400 text-sm max-w-[240px]">
                        We couldn't find programs within your current filters.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="hidden md:flex flex-col w-full md:max-w-md bg-white border-r border-slate-100 p-8">
                <div className="flex flex-col items-center mb-10">
                  ...
                  <div className="flex gap-4 mt-6">
                    <button
                      onClick={() => setActiveTab("events")}
                      className={`px-4 py-2 rounded-full font-bold text-sm ${activeTab === "events" ? "bg-[#004d33] text-white" : "text-slate-400 hover:bg-slate-50"}`}
                    >
                      Events Map
                    </button>
                    <button
                      onClick={() => setActiveTab("prayers")}
                      className={`px-4 py-2 rounded-full font-bold text-sm ${activeTab === "prayers" ? "bg-[#004d33] text-white" : "text-slate-400 hover:bg-slate-50"}`}
                    >
                      Prayer Times
                    </button>
                    <button
                      onClick={() => setActiveTab("quran")}
                      className={`px-4 py-2 rounded-full font-bold text-sm ${activeTab === "quran" ? "bg-[#004d33] text-white" : "text-slate-400 hover:bg-slate-50"}`}
                    >
                      Quran
                    </button>
                    <button
                      onClick={() => setActiveTab("about")}
                      className={`px-4 py-2 rounded-full font-bold text-sm ${activeTab === "about" ? "bg-[#004d33] text-white" : "text-slate-400 hover:bg-slate-50"}`}
                    >
                      <div className="flex items-center gap-2">
                        <Icons.Info />
                        <span>About</span>
                ...
                <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <p className="text-xs text-slate-500 mb-4">
                    Prayer times are calculated based on your selected city or
                    current location.
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="px-2 py-1 bg-white border border-slate-200 rounded text-xs hover:border-slate-800 transition-colors"
                    >
                      Current Location
                    </button>
          ...
          <div className="flex-1 h-full w-full bg-[#fdfcf6]">
            <ErrorBoundary>
              {quranView ? (
                ...
                <QuranIndex
                  onSelect={(mode, id) => setQuranView({ mode, id })}
